...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
    ...
    \path[yshift=0.5cm]
            node[hidden neuron] (H-1) at (\layersep * 0.65,-1 cm) {$\Sigma$};
    \foreach \i in {2,...,\hiddenlayernodes}
        \path[yshift=0.5cm, opacity=\opacity]
            node[hidden neuron] (H-\i) at (\layersep * 0.65,-\i cm) {$\Sigma$};

    % Draw the hidden neuron activation layer nodes
    \path[yshift=0.5cm]
        node[hidden neuron activation] (HA-1) at (\layersep * 1.1,-1 cm) {\tiny ReLU};
    \foreach \i in {2,...,\hiddenlayernodes}
        \path[yshift=0.5cm, opacity=\opacity]
            node[hidden neuron activation] (HA-\i) at (\layersep * 1.1,-\i cm) {\tiny ReLU};

    % Draw the output layer nodes
    \node[output neuron, right of=H-3] (O) {$\Sigma$};
    \node[output neuron, right of=H-3] (OA) at (\layersep * 1,-2.5cm) {\tiny ReLU};
